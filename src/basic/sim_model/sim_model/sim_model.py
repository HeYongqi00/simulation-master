...
class SimModel(Node):
    def __init__(self, name):
        ...
        self.x, self.y = GetLonData(file_name)
         # 初始化插值器
        # self.interpolator = NearestNDInterpolator(self.x, self.y) # by zzs
        self.interpolator = LinearNDInterpolator(self.x, self.y) # by zzs
        pass
    ...
    def UpdataState(self):
        ...
        if self.msgPid is not None:
            ...
            now_ts = time.time()
            # 插值操作
            inter_val_ = self.interpolator(np.array([[speed_now_, command_]]))
